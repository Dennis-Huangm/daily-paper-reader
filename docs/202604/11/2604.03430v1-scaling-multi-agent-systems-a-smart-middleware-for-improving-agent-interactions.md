...
tags: ["query:ag-rl"]
score: 6.0
evidence: 基于大语言模型的多智能体系统交互中间件
tldr: 针对大模型多智能体系统扩展中的上下文碎片化和通信低效问题，本文提出智能中间件“认知织网节点”（CFN）。CFN作为主动中介，利用强化学习优化拓扑选择、语义对齐、安全执行和提示词转换。该架构通过拦截并重写智能体间通信，在保持智能体轻量化的同时，显著提升了系统的协作一致性与性能。
source: arxiv
selection_source: fresh_fetch
motivation: 传统多智能体系统直接通信存在上下文断裂、安全性差及拓扑管理低效等局限，难以应对复杂持久的生态系统需求。
method: 提出认知织网节点（CFN）中间件，利用强化学习和优化算法主动拦截并重写智能体间通信，实现动态拓扑管理与语义对齐。
result: "在HotPotQA和MuSiQue数据集上的实验表明，CFN架构相比智能体间直接通信，性能提升了10%以上。"
conclusion: 将中间件从简单的消息传递提升为具备主动认知能力的层级，是实现大规模、高效且安全的多智能体协作的关键。
---

## 摘要
随着基于大语言模型（LLM）的多智能体系统（MAS）从实验性试点演变为复杂且持久的生态系统，智能体之间直接通信的局限性日益显现。当前的架构面临上下文碎片化、随机幻觉、僵化的安全边界以及低效的拓扑管理等问题。本文介绍了认知织网节点（Cognitive Fabric Nodes, CFN），这是一种在智能体之间创建无处不在的“认知织网”的新型中间件层。与传统的消息队列或服务网格不同，CFN 不仅仅是透传机制，而是主动的智能中介。该架构的核心是将记忆（Memory）从简单的存储提升为主动的功能基质，从而支撑四项关键能力：拓扑选择、语义对齐（Semantic Grounding）、安全策略执行和提示词转换。我们提出，这些功能中的每一项都由利用强化学习（RL）和优化算法的学习模块管理，以动态提高系统性能。通过拦截、分析和重写智能体间的通信，认知织网确保了单个智能体保持轻量化，同时使生态系统实现连贯性、安全性和语义对齐。我们在多智能体环境下的 HotPotQA 和 MuSiQue 数据集上评估了 CFN 的有效性，并证明与智能体间的直接通信相比，CFN 在这两个数据集上的性能提升均超过了 10%。
