...
tldr: 本文从统计学视角系统综述了人类反馈强化学习（RLHF）在大型语言模型对齐中的应用。针对RLHF中反馈数据具有噪声、主观性和异质性的挑战，文章将SFT、奖励建模和策略优化等核心步骤与Bradley-Terry-Luce模型、主动学习及不确定性量化等统计理论相结合。通过对比两阶段流水线与DPO等一阶段方法，并探讨RLAIF和可验证奖励等前沿扩展，本文为理解RLHF的统计基础提供了全面框架。
source: arxiv
...
motivation: 旨在解决RLHF在处理噪声和主观人类偏好反馈时面临的基础统计学挑战，以提升模型对齐的理论支撑。
method: 通过将RLHF组件映射到经典统计模型，系统回顾了奖励函数学习、策略优化算法以及新兴的一阶段对齐技术。
result: 梳理了从偏好建模到推理时算法的完整技术路径，并提供了包含基准数据集、评估协议及开源框架的综合资源。
conclusion: 明确了RLHF在统计推断和复杂反馈处理方面的核心地位，并指出了未来研究中亟待解决的开放性统计难题。
---
