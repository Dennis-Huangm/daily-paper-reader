...
tldr: 针对离线、零样本的信号时序逻辑（STL）规划问题，本文提出了GraSP-STL框架。该方法仅利用任务无关的离线数据集，通过学习目标条件价值函数构建状态空间的有向图抽象，将复杂的STL规划转化为图搜索问题。该框架实现了可重用的可达性学习与任务特定规划的解耦，支持在无需重新训练的情况下处理长时程、未见过的STL任务，并在多项实验中验证了其有效性。
selection_source: fresh_fetch
motivation: 现有的STL规划方法通常依赖环境交互或精确动力学模型，难以在仅有离线数据且任务未知的场景下实现零样本泛化。
method: 通过离线数据学习目标条件价值函数构建可达性图，将STL规划转化为基于算术几何平均鲁棒性评估的图搜索问题。
result: 实验结果证明该框架在多种离线STL规划任务中表现出色，能够有效处理长时程任务并实现对未见规格的零样本泛化。
conclusion: GraSP-STL通过解耦可达性学习与逻辑规划，为离线数据驱动的复杂逻辑任务规划提供了一种高效且通用的新途径。
---

## 摘要
本文研究了信号时序逻辑（STL）规范下的离线零样本规划。我们假设仅能访问由任务无关的行为策略收集的状态-动作-状态转移离线数据集，且不具备解析动力学模型，不进行进一步的环境交互，也不进行针对特定任务的重新训练。其目标是合成一种控制策略，使其生成的轨迹满足任意未见过的 STL 规范。为此，我们提出了 GraSP-STL，这是一个基于图搜索的框架，用于从离线轨迹中进行零样本 STL 规划。该方法从离线数据中学习目标条件价值函数，并利用其在状态空间上推导出有限时域可达性度量。基于该度量，它构建了一个抽象有向图，其节点表示代表性状态，边编码了可行的短时域转移。随后，规划被建模为对路标点序列的图搜索，使用算术-几何平均鲁棒性及其区间语义进行评估，并由学习到的目标条件策略执行。所提出的框架将可重用的可达性学习与任务条件规划相分离，从而实现了对未见 STL 任务的零样本泛化，并通过组合离线数据中的短时域行为实现长时域规划。实验结果证明了该方法在一系列离线 STL 规划任务中的有效性。
