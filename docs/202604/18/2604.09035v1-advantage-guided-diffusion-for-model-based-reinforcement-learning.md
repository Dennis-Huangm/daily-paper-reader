...
tags: ["query:ag-rl"]
score: 6.0
evidence: 基于优势引导扩散的模型强化学习智能体
tldr: 本文针对扩散世界模型在强化学习中存在的短视问题，提出了优势引导扩散（AGD-MBRL）。该方法利用优势函数估计来引导逆向扩散过程，使生成的轨迹向高长期回报区域集中。通过引入SAG和EAG两种引导机制，AGD-MBRL在不改变扩散模型训练目标的前提下，显著提升了在MuJoCo任务中的采样效率和最终性能，证明了优势感知引导在解决短时域预测问题上的有效性。
source: arxiv
...
motivation: 现有的扩散模型引导方式要么忽略价值信息，要么因仅依赖即时奖励而在短预测时域内表现出严重的短视性。
method: 提出AGD-MBRL框架，利用Sigmoid或指数优势引导机制（SAG/EAG）在逆向扩散过程中根据优势函数对状态轨迹进行重采样。
result: 在多个MuJoCo基准任务中，该方法的样本效率和最终回报均优于PolyGRAD及PPO等基准算法，部分场景性能提升达两倍。
conclusion: 优势感知引导是克服扩散模型MBRL短视缺陷的有效手段，能在保持模型训练简单的同时显著增强策略改进效果。
---

## 摘要
基于自回归世界模型的基于模型的强化学习（MBRL）面临复合误差问题，而扩散世界模型通过联合生成轨迹段来缓解这一问题。然而，现有的扩散引导要么仅基于策略（丢弃了价值信息），要么基于奖励（在扩散时界较短时会变得短视）。我们提出了用于 MBRL 的优势引导扩散（AGD-MBRL），它利用智能体的优势估计来引导反向扩散过程，使采样集中在预期能产生超出生成窗口更高长期回报的轨迹上。我们开发了两种引导方式：(i) Sigmoid 优势引导 (SAG) 和 (ii) 指数优势引导 (EAG)。我们证明，通过 SAG 或 EAG 引导的扩散模型允许我们对轨迹进行重加权采样，其权重随状态-动作优势的增加而增加，这在标准假设下意味着策略改进。此外，我们表明，与无引导的扩散模型相比，AGD-MBRL 生成的轨迹遵循改进后的策略（即具有更高的价值）。AGD 与 PolyGRAD 风格的架构无缝集成，通过引导状态分量同时保持动作生成受策略约束，且无需更改扩散训练目标。在 MuJoCo 控制任务（HalfCheetah、Hopper、Walker2D 和 Reacher）上，AGD-MBRL 在样本效率和最终回报方面优于 PolyGRAD、在线 Diffuser 风格的奖励引导以及无模型基准（PPO/TRPO），在某些情况下提升幅度达 2 倍。这些结果表明，优势感知引导是解决扩散模型 MBRL 中短时界短视问题的一种简单且有效的补救措施。
