...
tags: ["query:ag-rl"]
score: 6.0
evidence: 用于多机器人控制的演员-评论家学习
tldr: 针对具有非对称随机到达和切换延迟的多机器人多队列系统，本文提出了一种基于穷尽分配 Actor-Critic 学习的在线任务分配方法。该方法利用最优策略具有穷尽服务特性的结构化先验，通过强化学习仅学习空闲机器人的下一队列分配。实验表明，该方法在非对称环境下优于传统的 ESL 规则，能有效降低持有成本和队列长度，为实时多机器人调度提供了高效方案。
selection_source: fresh_fetch
motivation: 旨在解决多机器人系统中非对称任务到达和切换延迟带来的在线任务分配难题，克服传统启发式规则在非对称场景下的局限性。
method: 提出一种穷尽分配 Actor-Critic 架构，通过强制执行穷尽服务策略并仅对空闲机器人的目标队列进行学习优化。
result: 在多种服务器比例、负载和非对称到达配置下，该方法比 ESL 基准具有更低的折扣持有成本和更短的平均队列长度。
conclusion: 结合结构化先验的 Actor-Critic 方法能有效提升多机器人调度性能，是处理复杂随机排队系统的有力工具。
---

## 摘要
我们研究了具有非对称随机到达和切换延迟的多机器人、多队列系统的在线任务分配。我们在离散时间内对该问题进行了建模：每个位置在每个时隙最多只能容纳一个机器人，处理一项任务消耗一个时隙，在不同位置之间切换会产生一个时隙的移动延迟，且各位置的任务到达是具有异构速率的独立伯努利过程。基于我们之前的结构性研究结果（即最优策略属于穷尽型），我们构建了一个折扣成本马尔可夫决策过程，并开发了一种穷尽式分配 Actor-Critic 策略架构。该架构通过构造强制执行穷尽式服务，并仅学习空闲机器人的下一队列分配。与仅在对称条件下已知最优性的穷尽服务最长（ESL）队列规则不同，所提出的策略能够适应到达率的非对称性。在不同的服务器-位置比例、负载和非对称到达分布下，所提出的策略始终比 ESL 基准实现更低的折扣持有成本和更小的平均队列长度，同时在可获得最优基准的实例上保持接近最优。这些结果表明，结构感知 Actor-Critic 方法为实时多机器人调度提供了一种有效途径。
