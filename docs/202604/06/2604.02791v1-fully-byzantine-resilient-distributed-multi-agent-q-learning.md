...
tldr: 本文针对分布式多智能体强化学习在拜占庭攻击下难以收敛至最优解的问题，提出一种新型分布式Q学习算法。该算法利用两跳邻居信息的冗余过滤机制验证消息，确保智能体在边缘攻击下仍能以概率1收敛至最优价值函数。研究还给出了可多项式时间验证的拓扑条件，仿真证明了其在保障最优性方面的优越性。
selection_source: fresh_fetch
motivation: 现有的拜占庭容错多智能体强化学习方法往往只能收敛到次优解，难以在受损网络中保证学习到最优策略。
method: 提出一种基于冗余的过滤机制，利用两跳邻居信息校验入站消息，并引入了新的可验证网络拓扑条件。
result: 理论证明了该算法在拜占庭边缘攻击下能确保价值函数几乎处处收敛于最优解，且拓扑条件可在多项式时间内验证。
conclusion: 该研究实现了完全拜占庭容错的分布式Q学习，解决了分布式强化学习在恶意攻击下的最优性保障难题。
---

## 摘要
我们研究了拜占庭容错的分布式多智能体强化学习（MARL），其中智能体必须在受损的通信网络上协作学习最优价值函数。现有的容错 MARL 方法通常只能保证以概率 1 收敛到近优价值函数，或者需要严苛的假设才能确保收敛到最优解。因此，在这些方法下，智能体可能无法学习到最优策略。为了解决这一问题，我们提出了一种新型分布式 Q 学习算法，在该算法下，尽管存在拜占庭边攻击，所有智能体的价值函数仍能以概率 1 收敛到最优价值函数。其核心思想是一种基于冗余的过滤机制，该机制利用二跳邻居信息来验证传入消息，同时保持双向信息流。随后，我们为算法的收敛性引入了一种新的拓扑条件，提出了一种构建此类网络的系统方法，并证明了该条件可以在多项式时间内得到验证。我们通过仿真验证了研究结果，表明我们的方法能够收敛到最优解，而先前的方法在拜占庭边攻击下则会失效。
