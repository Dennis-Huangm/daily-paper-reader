...
tags: ["query:ag-rl"]
score: 8.0
evidence: 将 LLM 生成的协调先验集成到多智能体强化学习中
tldr: 本研究探讨了利用大语言模型（LLM）生成的图先验来优化多智能体强化学习（MARL）中的协作问题。针对现有方法依赖手动指定拓扑或数据密集型学习的局限，作者提出通过自然语言描述智能体观测，利用LLM推断潜在协作模式，并将其集成到基于图神经网络的MARL框架中。在MPE基准测试上的实验证明，即使是1.5B参数的小型LLM也能显著提升多智能体系统的协调能力和适应性。
source: arxiv
...
motivation: 传统多智能体协作方法依赖手动规则或大量数据学习拓扑，存在语义信息缺失、鲁棒性差且训练效率低的问题。
method: 利用LLM根据智能体观测的自然语言描述生成协调图先验，并将其通过图卷积层集成到基于GNN的MARL算法中。
result: 在多个MPE场景中，LLM生成的先验显著提升了协作性能，且实验表明1.5B参数规模的模型即可生成有效的图先验。
conclusion: LLM衍生的图先验能有效增强动态多智能体环境中的协调与适应能力，为结合语言常识与强化学习提供了新路径。
---
