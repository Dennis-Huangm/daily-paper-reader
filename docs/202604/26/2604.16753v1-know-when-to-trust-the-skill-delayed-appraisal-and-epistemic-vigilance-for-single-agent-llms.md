...
motivation: 传统路由方法在复杂工具环境下易受上下文污染影响，导致智能体出现过度推理和决策失误。
method: 提出MESA-S框架，利用元认知技能卡和延迟探测机制，将技能的效用感知与其高成本执行过程解耦。
result: 实验表明该架构能有效修剪不必要的推理循环，并缓解因依赖外部工具而产生的置信度膨胀问题。
conclusion: 显式编程信任溯源和延迟评估机制是构建具备认识警觉性、可靠单智能体架构的有效路径。
---
