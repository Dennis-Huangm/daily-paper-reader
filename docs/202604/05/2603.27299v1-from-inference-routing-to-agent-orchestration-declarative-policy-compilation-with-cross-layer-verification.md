...
tldr: 本研究针对LLM推理路由中的策略一致性挑战，将Semantic Router DSL从无状态请求路由扩展至多步智能体编排与基础设施部署。该DSL采用非图灵完备设计，通过统一编译器将单一声明式源文件转化为涵盖推理网关、编排框架（如LangGraph）及Kubernetes配置的验证节点。这种方法确保了跨层的信号定义一致性，消除了因跨团队协作导致的策略漂移，同时提供了结构化的审计追踪和冲突检测，显著提升了系统的可验证性与运维效率。
selection_source: fresh_fetch
motivation: 旨在解决LLM应用中推理网关、智能体编排与底层基础设施之间因策略定义不统一而导致的策略漂移和协作成本高昂的问题。
method: 扩展非图灵完备的声明式DSL，通过统一编译器将单一策略源编译为多层验证节点、K8s构件及协议网关配置。
result: 实现了路由逻辑的穷尽性验证、无冲突分支及跨层引用的完整性，确保阈值变更能同步传播至所有系统层级。
conclusion: 通过将推理路由与智能体编排统一在声明式策略框架下，该方案为构建可审计、低成本且高度一致的AI应用架构提供了有效路径。
---

## 摘要
Semantic Router DSL 是一种非图灵完备的策略语言，已在生产环境中部署，用于处理逐请求的大语言模型（LLM）推理路由：内容信号（如嵌入相似度、PII 检测、越狱评分）被输入到加权投影和按优先级排序的决策树中，以选择模型、执行隐私策略并生成结构化的审计追踪——所有这些均来自单一的声明式源文件。先前的工作为概率谓词建立了无冲突编译，并将该 DSL 置于“工作负载-路由器-池”（Workload-Router-Pool）推理架构中。本文将该语言从无状态的逐请求路由扩展到多步智能体工作流，涵盖了从推理网关到智能体编排再到基础设施部署的全路径。该 DSL 编译器可从同一源码生成用于编排框架（LangGraph、OpenClaw）的经验证决策节点、Kubernetes 构件（NetworkPolicy、Sandbox CRD、ConfigMap）、YANG/NETCONF 负载以及协议边界网关（MCP、A2A）。由于该语言是非图灵完备的，编译器保证了穷尽路由、无冲突分支、引用完整性以及与决策逻辑结构耦合的审计追踪。由于信号定义在不同目标之间共享，阈值更改可以在一个编译步骤中从推理网关传播到智能体网关再到基础设施构件，从而消除了作为策略漂移主要来源的跨团队协作。我们将该方法建立在四个支柱之上——可审计性、成本效率、可验证性和可调优性，并识别了每一层的验证边界。
