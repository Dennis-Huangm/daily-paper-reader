...
tldr: 本研究针对自动化科研中多智能体系统（MAS）的协作框架进行了系统性实证研究。通过在受控测试平台上对比单智能体与两种多智能体架构（子智能体并行探索与专家团队协作），探讨了它们在机器学习优化任务中的表现。研究揭示了操作稳定性与理论深度之间的权衡，为未来设计可根据任务复杂度动态调整协作结构的自动化科研系统提供了重要指导。
selection_source: fresh_fetch
motivation: 随着AI智能体从单模型向多智能体系统演进，如何确定自动化科研中最优的智能体协作框架成为研究空白。
method: 在配备Git隔离和全局内存的受控测试平台上，对比了单智能体、子智能体并行架构和专家团队协作架构在固定计算预算下的执行效果。
result: 研究发现子智能体模式在限时任务中表现出高韧性和高吞吐量，而专家团队架构在长时计算下能实现更深层的理论对齐和架构重构。
conclusion: 未来自动化科研系统应采用动态路由架构，根据任务的实时复杂性灵活调整其多智能体协作模式。
---

## 摘要
随着人工智能智能体（AI agents）的发展，研究界正迅速从单一大型语言模型（LLMs）转向多智能体系统（MAS），以克服自动化研究中的认知瓶颈。然而，针对这些自主智能体的最优多智能体协调框架在很大程度上仍未得到探索。在本文中，我们提出了一项系统的实证研究，调查了不同多智能体结构在自动化机器学习优化中的比较效能。利用一个配备了 Git 工作树隔离和显式全局内存的、受严格控制的基于执行的测试平台，我们将单智能体基准与两种多智能体范式进行了对比：子智能体架构（具有事后整合的并行探索）和智能体团队架构（具有执行前交接的专家）。通过在严格固定的计算时间预算下评估这些系统，我们的研究结果揭示了运行稳定性与理论深思熟虑之间的基本权衡。子智能体模式充当了一个高弹性、高吞吐量的搜索引擎，最适合在严格的时间限制下进行广泛且浅层的优化。相反，由于多作者代码生成，智能体团队拓扑结构表现出更高的运行脆弱性，但在延长计算预算的情况下，它能实现复杂架构重构所需的深度理论对齐。这些实证见解为设计未来的自动研究系统提供了可操作的指南，提倡采用能够根据实时任务复杂度调整其协作结构的动态路由架构。
