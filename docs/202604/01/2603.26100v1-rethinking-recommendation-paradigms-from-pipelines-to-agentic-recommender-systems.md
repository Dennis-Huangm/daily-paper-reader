...
tldr: 针对传统推荐系统多阶段流水线存在的静态黑盒、依赖人工调优且难以扩展等问题，本文提出了智能体推荐系统（AgenticRS）范式。该范式将推荐模块重构为具备独立评估和自我进化能力的智能体，通过强化学习和大语言模型驱动的架构生成实现模型进化，并利用分层奖励机制协调局部与全局目标，为构建自进化推荐系统提供了新的蓝图。
selection_source: fresh_fetch
motivation: 传统推荐系统流水线过于静态且高度依赖人工工程，难以在异构数据和多目标约束下实现高效的规模化扩展。
method: 提出将推荐模块转化为具备闭环评估和进化空间的智能体，利用强化学习和LLM生成技术驱动模型架构及组合方式的自动演进。
result: 构建了一套支持个体与组合进化的演进机制，并通过内外层奖励设计实现了局部模块优化与全局业务目标的有效耦合。
conclusion: 该研究为推荐系统从手动工程驱动向自动化、自进化的智能体范式转型提供了清晰的理论框架和实施路径。
---

## 摘要
大规模工业级推荐系统通常采用固定的多阶段流水线（召回、排序、重排），并已从协同过滤演进到深度模型及大型预训练模型。然而，无论是多阶段设计还是所谓的“统一模型”（One Model）设计，本质上仍然是静态的：模型是黑盒，系统改进依赖于人工假设和工程，这在异构数据和多目标业务约束下难以扩展。我们提出了一种智能体推荐系统（AgenticRS），将关键模块重新组织为智能体（agents）。只有当模块形成功能闭环、可独立评估且具备可进化的决策空间时，才会被提升为智能体。针对模型智能体，我们概述了两种自进化机制：在定义明确的动作空间中进行强化学习风格的优化，以及在开放式设计空间中基于大语言模型生成和选择新的架构与训练方案。我们进一步区分了单个智能体的个体进化与涉及多个智能体如何选择和连接的组合进化，并采用分层的内外奖励设计，将局部优化与全局目标相结合。这为将静态流水线转变为自进化的智能体推荐系统提供了一个简洁的蓝图。
