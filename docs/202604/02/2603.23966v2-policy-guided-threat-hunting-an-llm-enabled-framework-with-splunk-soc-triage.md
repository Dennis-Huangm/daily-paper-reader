...
tldr: 针对高级持续性威胁（APT）和SOC分析师面临的海量日志挑战，本文提出了一种集成Agentic AI与Splunk的自动化威胁狩猎框架。该框架结合了重构自编码器进行异常评估、双层深度强化学习（DRL）进行初步分拣，以及大语言模型（LLM）进行上下文分析。实验证明，该框架能自主适应不同SOC目标，有效识别恶意流量并辅助决策，显著提升了网络安全防御的自动化水平。
selection_source: fresh_fetch
motivation: 传统安全方案难以应对不断演进的APT威胁，且SOC分析师难以处理来自多样化设备的海量日志。
method: 提出一个集成Splunk、自编码器异常检测、双层深度强化学习分拣及LLM上下文分析的Agentic AI框架。
result: 实验表明该框架能有效适应不同SOC目标，准确识别可疑与恶意流量，并辅助分析师进行流量管控决策。
conclusion: 该研究通过结合AI与SIEM平台，为网络安全决策提供了创新的自动化威胁狩猎方案，增强了应对演进威胁的能力。
---

## 摘要
随着网络空间中高级持续性威胁（APT）的不断演变，传统的安全解决方案已不足以满足组织的威胁狩猎需求。此外，安全运营中心（SOC）分析师往往不堪重负，难以分析来自组织内各种设备的海量日志。为了应对这些挑战，我们提出了一种自动化且动态的威胁狩猎框架，用于监控演变中的威胁、适应不断变化的网络状况，并对可疑和恶意流量的缓解进行基于风险的优先级排序。通过将代理式人工智能（Agentic AI）与成熟的 SIEM 平台 Splunk 相结合，我们开发了一个独特的威胁狩猎框架。该框架系统且无缝地集成了不同的威胁狩猎模块，涵盖了从流量摄取到使用基于重构的自动编码器进行异常评估、使用双层深度强化学习（DRL）进行初步分诊，以及使用大语言模型（LLM）进行上下文分析。我们在公开基准数据集和模拟数据集上对该框架进行了评估。实验结果表明，该框架能够自主地有效适应不同的 SOC 目标，并识别可疑和恶意流量。该框架通过支持 SOC 分析师在阻断、允许或监控网络流量方面的决策，提升了运营效率。本研究通过提出一种用于安全决策的新型威胁狩猎框架，丰富了网络安全和威胁狩猎领域的文献，并促进了开发更有效框架以应对不断演变的网络威胁的持续研究工作。
