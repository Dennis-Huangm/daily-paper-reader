...
tags: ["query:ag-rl"]
score: 7.5
evidence: 针对部分可观测博弈中独立学习的多智能体强化学习框架
tldr: 本研究针对部分可观测马尔可夫博弈（POMG）中纳什均衡学习的复杂性问题，提出了一种独立学习算法。在状态转移独立且底层为势博弈的假设下，该算法允许智能体仅凭自身观测和动作，在无通信的情况下收敛至近似纳什均衡。通过引入有限历史窗口策略和滤波器稳定性假设，成功将计算和样本复杂度降低至拟多项式级别，克服了传统方法中随智能体数量指数增长的难题。
source: arxiv
...
motivation: 解决部分可观测马尔可夫博弈中，现有算法依赖中心化信息且复杂度随智能体数量指数级增长的问题。
method: 提出一种基于有限历史窗口策略的独立学习算法，利用滤波器稳定性将POMG近似为近势马尔可夫博弈。
result: 证明了在无通信和仅有局部观测的情况下，智能体能够联合收敛至近似纳什均衡，且复杂度为拟多项式级。
conclusion: 该研究为大规模部分可观测环境下的多智能体强化学习提供了一种高效且去中心化的理论框架。
---
