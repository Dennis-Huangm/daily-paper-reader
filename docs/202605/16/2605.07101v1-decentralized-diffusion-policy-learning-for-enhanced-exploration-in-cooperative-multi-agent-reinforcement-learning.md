...
## 摘要
协作式多智能体强化学习（MARL）涉及复杂的智能体交互，需要有效的探索策略。一类著名的 MARL 算法——分散式 Softmax 策略梯度（DecSPG），通过基于能量的策略更新来解决这一问题。然而在实践中，这类基于能量的策略难以维护，通常被投影到高斯策略类中。在这项工作中，我们证明了高斯策略有限的表达能力严重阻碍了 DecSPG 中的探索，且这种局限性随着智能体数量的增加而加剧。为了解决这个问题，我们提出了分散式扩散策略学习（DDPL），它使用去噪扩散概率模型来参数化每个智能体的策略。这是一种具有强大表达能力的生成模型，能够捕捉多峰动作分布以增强探索。DDPL 通过重要性采样评分匹配（ISSM）实现了扩散策略的高效在线训练，这是一种具有理论保证的新型训练方法。我们在具有代表性的连续动作 MARL 基准测试上评估了 DDPL，包括多智能体粒子环境、多智能体 MuJoCo、IsaacLab 以及 JAX 重新实现的星际争霸多智能体挑战赛，并观察到了持续的性能提升。
