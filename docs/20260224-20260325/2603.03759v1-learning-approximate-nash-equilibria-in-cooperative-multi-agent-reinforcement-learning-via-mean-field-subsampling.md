...
tags: ["query:ag-rl"]
score: 7.0
evidence: 大规模系统中的协作式多智能体强化学习
tldr: 针对大规模多智能体系统中全局决策者面临的观测受限问题，本文研究了包含一个全局智能体和n个同质局部智能体的合作马尔可夫博弈。提出了一种名为ALTERNATING-MARL的交替学习框架，通过子采样平均场Q学习和局部策略优化，证明了算法能收敛至近似纳什均衡，并实现了状态与动作空间样本复杂度的分离，在多机器人控制等场景中表现优异。
selection_source: fresh_fetch
motivation: 旨在解决大规模网络化控制系统中，全局决策者在仅能观察部分局部状态的严格限制下与海量智能体协作的难题。
method: 提出ALTERNATING-MARL框架，让全局智能体进行子采样平均场Q学习，而局部智能体在诱导的MDP中优化策略。
result: 理论证明该动态过程收敛至O(1/sqrt(k))近似纳什均衡，并实现了联合状态与动作空间样本复杂度的有效分离。
conclusion: 该研究为受限观测下的大规模多智能体强化学习提供了具有理论保障的高效算法，并在多机器人控制和联邦优化中得到了验证。
---

## 摘要
许多大规模平台和网络化控制系统包含一个中心化决策者，其在严格的可观测性约束下与海量智能体群体进行交互。受此类应用启发，我们研究了通信受限机制下，由一个全局智能体和 $n$ 个同质局部智能体组成的合作式马尔可夫博弈，其中全局智能体在每个时间步仅观测 $k$ 个局部智能体状态的子集。我们提出了一种交替学习框架 (ALTERNATING-MARL)，在该框架中，全局智能体针对固定的局部策略执行子采样平均场 Q 学习，而局部智能体通过在诱导马尔可夫决策过程 (MDP) 中进行优化来更新。我们证明了这些近似最佳响应动力学收敛至 $\widetilde{O}(1/\sqrt{k})$ 近似纳什均衡，同时实现了联合状态空间与动作空间样本复杂度的分离。最后，我们在多机器人控制和联邦优化的数值模拟中验证了我们的结果。
