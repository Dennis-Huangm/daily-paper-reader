...
## 摘要
具有多任务数据集的离线多智能体强化学习（MARL）面临挑战，原因在于不同任务中的智能体数量各异，且需要泛化到未见场景。先前的研究采用具有观测标记化（observation tokenization）和分层技能学习的 Transformer 来解决这些问题。然而，这些方法未能充分利用 Transformer 的注意力机制进行智能体间的协作，且依赖于单一的历史标记（history token），这限制了它们在部分可观测 MARL 环境中捕捉长程时间依赖的能力。在本文中，我们提出了 STAIRS-Former，这是一种增强了空间和时间层级的 Transformer 架构，能够在捕捉长交互历史的同时，对关键标记进行有效的注意力分配。我们进一步引入了标记丢弃（token dropout），以增强在不同智能体群体规模下的鲁棒性和泛化能力。在包括 SMAC、SMAC-v2、MPE 和 MaMuJoCo 在内的多种多智能体基准测试及多任务数据集上的广泛实验表明，STAIRS-Former 一贯优于先前的方法，并达到了新的最先进性能。
