...
motivation: 传统的模仿学习无法让智能体理解动作背后的逻辑，而现有的反思训练往往只是在模仿预设文本而非进行真正的自主推理。
method: 采用强化学习框架，通过奖励模型对动作优劣的正确判断，引导模型自主学习如何对比不同动作并产生批判性反思。
result: ACT在多个基准测试上比模仿学习平均提升5.07分，比强化学习提升4.62分，且在未见过的任务和通用推理上均有显著进步。
conclusion: ACT证明了通过训练模型进行动作评价和批判，可以有效提升LLM智能体的自主推理能力和任务执行成功率。
---
