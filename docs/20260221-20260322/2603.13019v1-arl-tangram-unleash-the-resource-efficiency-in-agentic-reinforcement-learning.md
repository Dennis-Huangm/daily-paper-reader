...
tags: ["query:ag-rl"]
score: 10.0
evidence: Agentic强化学习的资源管理系统
tldr: 针对代理强化学习（Agentic RL）中外部资源（如CPU和GPU）利用率低下的问题，本文提出了ARL-Tangram系统。该系统引入了动作级编排机制，通过统一的动作表述和弹性调度算法实现细粒度的资源共享与动态扩展。实验证明，ARL-Tangram显著缩短了动作完成时间并大幅节省了资源，目前已成功应用于MiMo系列模型的训练。
selection_source: fresh_fetch
motivation: 现有的代理强化学习框架通常采用静态资源预留，导致外部计算资源在处理复杂交互任务时存在严重的效率瓶颈和浪费。
method: 提出动作级编排架构，利用弹性调度算法最小化动作完成时间，并针对异构资源特性设计了专门的管理模块。
result: "该系统将平均动作完成时间提升了4.3倍，强化学习训练步长加速1.5倍，同时节省了高达71.2%的外部资源消耗。"
conclusion: ARL-Tangram通过精细化的资源调度释放了代理强化学习的性能潜力，是支持大规模智能体模型高效训练的关键基础设施。
---
