...
tags: ["query:ag-rl"]
score: 10.0
evidence: 带有内在反馈的LLM智能体在线强化学习框架
tldr: RetroAgent是一个在线强化学习框架，旨在通过回顾性双重内在反馈提升LLM智能体的进化能力。针对传统RL仅优化外部奖励导致探索不足和经验利用率低的问题，该框架引入了后验自我反思机制，结合数值反馈（追踪子任务进度）和语言反馈（提炼可复用经验），并利用SimUtil-UCB策略检索记忆。实验证明其在多个复杂任务中达到SOTA，显著优于现有RL和元学习方法。
selection_source: fresh_fetch
motivation: 传统强化学习智能体过度依赖外部奖励，导致探索受限且难以从历史经验中有效学习和进化。
method: 提出结合数值反馈（衡量增量进度）与语言反馈（提炼经验教训）的双重内在反馈机制，并配合SimUtil-UCB策略进行记忆检索。
result: "在ALFWorld、WebShop等四个挑战性任务中，RetroAgent性能大幅超越GRPO等基准模型，最高提升达27.1%。"
conclusion: 通过将回顾性反思与强化学习结合，RetroAgent实现了从单纯解决任务到持续自我进化的转变，增强了泛化与适应能力。
---

## 摘要
针对基于大语言模型（LLM）智能体的标准强化学习（RL）通常优化外部任务成功奖励，优先考虑一次性任务解决而非持续适应。因此，由于探索有限，智能体可能会收敛到次优策略，且积累的经验隐式地存储在模型参数中，阻碍了高效的经验学习。受人类回顾性自我提升能力的启发，我们提出了 RetroAgent，这是一个在线强化学习框架，使智能体不仅能通过解决任务，还能在外部任务成功奖励和回顾性双重内在反馈的共同指导下通过进化来掌握复杂的交互环境。具体而言，RetroAgent 具有一种事后自我反思机制，可产生：(1) 内在数值反馈，通过追踪相对于先前尝试的增量式子任务完成情况来奖励有前景的探索；(2) 内在语言反馈，将可重用的教训提炼到记忆缓冲区中，并通过我们提出的相似度与效用感知上置信界（SimUtil-UCB）策略进行检索，从而共同平衡相关性、效用和探索。在四个具有挑战性的智能体任务上的广泛实验表明，RetroAgent 达到了当前最先进（SOTA）的性能，显著优于 RL 微调、记忆增强 RL、探索引导 RL 和元 RL 方法——例如，在 ALFWorld 上比经过群体相对策略优化（GRPO）训练的智能体高出 18.3%，在 WebShop 上高出 15.4%，在 Sokoban 上高出 27.1%，在 MineSweeper 上高出 8.9%——同时保持了强大的测试时适应能力和分布外泛化能力。
