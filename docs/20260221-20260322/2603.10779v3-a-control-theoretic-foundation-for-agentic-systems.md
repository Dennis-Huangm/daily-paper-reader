...
## 摘要
本文开发了一个控制理论框架，用于分析嵌入在反馈控制回路中的代理系统。在这些系统中，AI 代理可以在运行过程中调整控制器参数、选择控制策略、调用外部工具、重构决策架构以及修改控制目标。通过将代理性（agency）解释为对控制架构要素的层级化运行时决策权，这些能力得到了形式化表达，从而产生了一种增强的闭环表示，其中物理状态、内部记忆、工具输出、交互信号和设计变量作为一个耦合动力系统共同演化。文中定义了一个五级代理层级，涵盖了从固定控制律到控制架构和目标的运行时合成。分析表明，代理性的增加引入了相互作用的动力学机制，如时变自适应、内生切换、决策诱导延迟和结构重构。该框架在非线性和线性环境下均得到了开发，为安全关键型应用中的 AI 赋能控制系统提供了明确的设计约束。
