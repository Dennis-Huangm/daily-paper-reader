...
title_zh: MOSAIC：一个用于同质与异质多智能体强化学习、大语言模型、视觉语言模型及人类决策者跨范式比较与评估的统一平台
...
tags: ["query:ag-rl"]
score: 8.0
evidence: 用于强化学习和LLM智能体跨范式比较的统一平台
...
method: 引入基于IPC的工人协议和算子抽象来统一不同框架的接口，并提供手动与脚本驱动的确定性评估框架。
result: 成功实现了一个支持多种异构智能体在相同环境下进行即时组队、行为对比和可复现实验的视觉化平台。
conclusion: MOSAIC为跨范式的多智能体研究提供了统一的基础设施，有效促进了RL、LLM和人机交互社区的融合与协作。
---

## 摘要
强化学习（RL）、大语言模型（LLM）和视觉语言模型（VLM）已被广泛地独立研究。然而，现有的基础设施缺乏在同一环境中部署来自不同决策范式的智能体的能力，这使得在混合多智能体设置中研究它们，或在相同条件下公平地比较它们的行为变得十分困难。我们提出了 MOSAIC，这是一个开源平台，通过整合多种现有的强化学习环境，并使异质智能体（RL 策略、LLM、VLM 和人类玩家）能够在即时（ad-hoc）团队设置中运行并获得可复现的结果，从而填补了这一空白。MOSAIC 提出了三项贡献：(i) 一种基于进程间通信（IPC）的工作节点协议，将原生和第三方框架封装为隔离的子进程工作节点，每个节点在不修改的情况下执行其原生的训练和推理逻辑，并通过版本化的进程间协议进行通信；(ii) 一种算子（operator）抽象，通过将工作节点映射到智能体来形成智能体级接口：无论是由 RL 策略、LLM 还是人类支持，每个算子都遵循一个最小化的统一接口；(iii) 一个确定性的跨范式评估框架，提供两种互补模式：手动模式，在共享种子的前提下以同步步进（lock-step）方式推进多达 N 个并发算子，以便对行为差异进行细粒度的视觉检查；脚本模式，通过声明式 Python 脚本驱动自动化的长时运行评估，以实现可复现的实验。我们将 MOSAIC 作为一个开放的、视觉优先的平台发布，以促进 RL、LLM 和人机协同（human-in-the-loop）社区中可复现的跨范式研究。
