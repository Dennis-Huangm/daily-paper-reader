...
tags: ["query:ag-rl"]
score: 8.0
evidence: 在多智能体策略中用LLM取代强化学习Oracle
tldr: 针对多智能体强化学习中策略空间响应预言机（PSRO）生成的神经网络策略具有“黑盒”特性、难以解释的问题，本文提出了代码空间响应预言机（CSRO）。该框架利用大语言模型（LLM）将最优响应计算转化为代码生成任务，直接生成可读性强的策略代码。通过零样本提示、迭代优化及AlphaEvolve进化系统，CSRO在保持竞争力的同时，实现了策略的高度可解释性与多样性。
selection_source: fresh_fetch
motivation: 传统多智能体强化学习生成的神经网络策略缺乏透明度，导致其难以被人类理解、信任或进行调试。
method: 提出CSRO框架，将LLM作为策略预言机，通过代码生成、迭代改进和分布式进化系统来合成可解释的算法策略。
result: 实验结果显示，CSRO在多个领域达到了与传统强化学习基准相当的性能，并产出了多样化且易于解释的策略。
conclusion: 该研究通过将策略优化转向可解释的代码合成，为多智能体博弈均衡的计算提供了一种更具透明度和信任感的新范式。
---
