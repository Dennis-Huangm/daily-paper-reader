...
tags: ["query:ag-rl"]
score: 10.0
evidence: 强化学习训练基于LLM的智能体并结合经验
tldr: "针对LLM智能体在强化学习中样本效率低且难以有效利用历史经验的问题，本文受神经科学启发提出“互补强化学习”（Complementary RL）。该方法通过在优化循环中实现经验提取器与策略执行器的协同演化，使提取器能根据执行器的成长动态调整经验管理策略，解决了经验与模型能力脱节的痛点。实验表明，该方法在单任务中性能提升10%，并在多任务场景下展现出良好的扩展性。"
selection_source: fresh_fetch
motivation: 现有方法中提取的经验往往是静态的或无法与不断进步的执行器同步演化，导致经验效用随训练进行而逐渐降低。
method: 提出一种协同演化机制，执行器由稀疏奖励优化，而经验提取器则根据其提供的经验对执行器成功的贡献度进行动态优化。
result: "在单任务场景下比传统强化学习基线提升了10%的性能，并在多任务设置中表现出强大的可扩展性。"
conclusion: Complementary RL 证明了经验提取器与策略执行器的同步演化是实现高效经验驱动型智能体学习的关键范式。
---
