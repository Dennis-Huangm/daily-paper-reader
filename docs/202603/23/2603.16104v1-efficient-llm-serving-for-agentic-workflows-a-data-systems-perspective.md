...
score: 7.0
evidence: 代理工作流的服务框架
tldr: 智能体工作流由一系列相互依赖的LLM调用组成，存在大量提示词重叠和中间结果冗余。现有系统如vLLM侧重于单次推理优化，忽视了跨调用依赖。本文提出Helium框架，从数据系统视角将工作流建模为查询计划，将LLM调用视为一级算子。通过集成主动缓存和缓存感知调度，Helium实现了Prompt和KV状态的高效复用，为LLM智能体提供了端到端的优化方案。
selection_source: fresh_fetch
motivation: 现有LLM服务系统忽视了智能体工作流中跨调用间的依赖关系和数据冗余，导致推理效率低下。
method: 提出Helium框架，将智能体工作流视为查询计划，并利用主动缓存和缓存感知调度技术最大化KV缓存的复用。
result: 实验表明，Helium在多种工作负载下比现有的先进智能体服务系统提升了高达1.56倍的运行速度。
conclusion: 跨工作流的端到端优化对于构建高效且可扩展的LLM智能体系统至关重要。
---
