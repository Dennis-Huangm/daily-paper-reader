...
motivation: 现有的LLM评分训练方法要么忽略了分数的数值连续性，要么缺乏对最优推理路径的探索。
method: 提出REAL框架，利用广义策略梯度将优化分解为思维链轨迹探索和回归感知的最终得分修正。
result: 在多尺度模型上均优于SFT和标准RL，在Qwen3-32B上相比SFT基准显著提升了Pearson和Spearman相关系数。
conclusion: 将回归目标集成到强化学习的探索过程中，是提升LLM作为自动评估器准确性和泛化性的关键。
---
