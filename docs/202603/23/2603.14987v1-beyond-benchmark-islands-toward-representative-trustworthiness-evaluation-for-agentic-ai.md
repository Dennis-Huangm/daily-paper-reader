...
title_zh: 超越基准孤岛：迈向智能体 AI 的代表性可信性评估
...
## 摘要
随着智能体 AI 系统从静态问答转向开放式、工具增强且多步骤的现实世界工作流，其权限的提升带来了更大的系统滥用和运行故障风险。然而，目前的评估实践仍然是碎片化的，仅在狭隘定义的设置中测量编程、幻觉、抗越狱能力或工具使用等孤立能力。我们认为，核心局限性不仅在于评估维度的覆盖不足，还在于缺乏原则性的代表性概念：智能体的可信性应该在具有代表性的社会技术场景分布上进行评估，而不是在一组相互脱节的基准实例上进行评估。为此，我们提出了全息智能体评估框架（HAAF），这是一种系统化的评估范式，在涵盖任务类型、工具接口、交互动态、社会背景和风险水平的场景流形上刻画智能体的可信性。该框架集成了四个互补组件：(i) 静态认知与策略分析，(ii) 交互式沙盒模拟，(iii) 社会伦理对齐评估，以及 (iv) 分布感知代表性采样引擎，该引擎共同优化了覆盖范围和风险敏感性——特别是针对传统基准系统性忽视的罕见但后果严重的尾部风险。这些组件通过一个迭代的可信优化工厂相连接。通过红队探测和蓝队加固的循环，该范式逐步缩小漏洞以满足部署标准，将智能体评估从基准孤岛转向具有代表性的现实世界可信性。示例实例的代码和数据可在 https://github.com/TonyQJH/haaf-pilot 获取。
