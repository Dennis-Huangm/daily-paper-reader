...
tldr: 本研究针对海运物流的减排与公平性挑战，提出EcoFair-CH-MARL框架。该框架通过原对偶预算层确保随机环境下的排放达标，利用公平感知奖励转换器平衡异构船队的成本，并采用两级分层架构实现智能体规模的线性扩展。理论证明其具有O(√T)的约束违反与公平损失遗憾界。实验表明，该方法在降低排放、提高吞吐量及增强公平性方面显著优于现有SOTA算法，为大规模社会责任型多智能体协作提供了新方案。
selection_source: fresh_fetch
motivation: 旨在解决海运物流中如何在满足严格碳排放预算的同时，确保异构智能体间资源分配的公平性与系统扩展性。
method: 结合原对偶优化、公平奖励转换器与分层解耦策略，构建了一个支持实时约束和公平性保障的约束分层多智能体强化学习框架。
result: "实验显示该方法在海运数字孪生场景下降低了15%的排放，提升了12%的吞吐量，且公平成本优化效果提升了45%。"
conclusion: 该研究证明了在复杂安全关键领域实现大规模、合规且兼顾公平的多智能体协同调度是可行且高效的。
---

## 摘要
全球脱碳目标和日益紧缩的市场压力要求海运物流解决方案同时具备高效性、可持续性和公平性。我们提出了 EcoFair-CH-MARL，这是一种受限分层多智能体强化学习框架，它集成了三项创新：(i) 一个原对偶预算层，可在随机天气和需求下证明性地限制累积排放；(ii) 一个具有动态调度惩罚的公平感知奖励转换器，在异构船队之间强制执行最大-最小成本公平；以及 (iii) 一种两层策略架构，将战略路由与实时船舶控制解耦，从而实现智能体数量的线性扩展。新的理论结果确立了约束违反和公平性损失的 O(√T) 遗憾界。在由自动识别系统（AIS）轨迹驱动的高保真海运数字孪生（16个港口，50艘船舶）以及能源网案例研究中的实验表明，与最先进的分层和受限 MARL 基准相比，其排放量降低了高达 15%，吞吐量提高了 12%，公平成本改善了 45%。此外，EcoFair-CH-MARL 比特定于公平性的 MARL 基准（如 SOTO、FEN）实现了更强的公平性（更低的基尼系数和更高的最大-最小福利），且其模块化设计与基于策略和基于价值的学习器均兼容。因此，EcoFair-CH-MARL 提升了在大规模、符合监管且具有社会责任感的安全关键领域中进行多智能体协作的可行性。
