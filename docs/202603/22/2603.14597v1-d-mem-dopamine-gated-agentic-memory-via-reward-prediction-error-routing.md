...
method: 引入基于RPE的快慢路由系统，利用轻量级评论器识别惊喜和效用，仅对高价值信息触发记忆重构。
result: "D-MEM在减少超过80% Token消耗的同时，消除了计算瓶颈，并在多步推理和抗干扰能力上优于基准模型。"
conclusion: 通过选择性地门控认知重构过程，D-MEM为构建高效、可扩展的终身学习智能体记忆提供了新方案。
---

## 摘要
自主大语言模型（LLM）智能体需要结构化的长期记忆，然而目前的“追加与演化”系统（如 A-MEM）面临 O(N^2) 的写入延迟和过高的 Token 成本。我们提出了 D-MEM（多巴胺门控智能体记忆），这是一种受生物学启发的架构，通过基于奖励预测误差（RPE）的快/慢路由系统，将短期交互与认知重构解耦。一个轻量级的评论家路由器（Critic Router）评估刺激的“惊讶度”（Surprise）和“效用”（Utility）。常规的、低 RPE 的输入会被绕过或缓存在 O(1) 的快速访问缓冲区中。相反，高 RPE 输入（如事实矛盾或偏好转变）会触发“多巴胺”信号，激活 O(N) 记忆演化流水线，以重塑智能体的知识图谱。为了在现实条件下评估性能，我们引入了 LoCoMo-Noise 基准测试，该基准在长期会话中注入受控的对话噪声。评估结果表明，D-MEM 减少了超过 80% 的 Token 消耗，消除了 O(N^2) 瓶颈，并在多跳推理和对抗韧性方面优于基准模型。通过选择性地对认知重构进行门控，D-MEM 为终身智能体记忆提供了一个可扩展且具有成本效益的基础。
