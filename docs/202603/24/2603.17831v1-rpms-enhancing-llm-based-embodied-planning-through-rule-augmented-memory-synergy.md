...
tags: ["query:ag-rl"]
score: 7.0
evidence: 增强智能体中基于 LLM 的具身规划
tldr: 针对大语言模型在具身智能规划中因动作前提条件严格和反馈稀疏导致的无效动作生成及状态漂移问题，本文提出RPMS架构。该架构通过结构化规则检索确保动作可行性，利用轻量级信念状态过滤记忆，并采用规则优先的仲裁机制解决冲突。实验表明，RPMS在ALFWorld和ScienceWorld基准测试中显著提升了不同规模模型的成功率，证明了规则与记忆协同在复杂环境规划中的有效性。
selection_source: fresh_fetch
motivation: 解决具身智能体在闭环环境中因动作前提不满足和状态感知偏差导致的规划失败循环。
method: 提出一种结合结构化规则检索、基于信念状态的记忆过滤以及规则优先仲裁机制的冲突管理架构。
result: 在ALFWorld任务中，RPMS显著提升了Llama 3.1 8B和Claude Sonnet 4.5的成功率，且在ScienceWorld中表现出良好的泛化性。
conclusion: 显式动作规则的约束和基于当前状态的记忆过滤是提升大模型具身规划稳定性和成功率的关键。
---
