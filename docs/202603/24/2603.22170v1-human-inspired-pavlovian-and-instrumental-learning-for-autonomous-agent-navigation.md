...
tldr: 针对自主智能体在不确定环境中平衡快速响应与目标导向规划的难题，本文提出一种受人类神经科学启发的混合强化学习架构。该架构整合了巴甫洛夫学习、无模型（MF）和基于模型（MB）的工具性学习，利用环境特征作为条件刺激来引导决策，并通过贝叶斯仲裁机制动态调节策略。实验证明，该方法显著提升了学习速度与安全性，实现了从探索到高效规划的平滑过渡。
selection_source: fresh_fetch
motivation: 旨在解决传统强化学习在复杂不确定环境下收敛慢、安全性差以及计算成本与模型准确性难以兼顾的问题。
method: 提出一种集成巴甫洛夫条件反射、MF和MB学习的混合架构，并引入动机信号和贝叶斯仲裁机制来动态融合不同决策路径。
result: 仿真结果显示，该方法加速了学习收敛，提高了导航安全性，并有效减少了智能体在高度不确定区域的停留。
conclusion: 证明了生物启发式模块化设计能显著增强自主系统在不确定环境下的鲁棒性与自适应能力。
---

## 摘要
在不确定环境中运行的自主智能体必须在快速响应与目标导向规划之间取得平衡。经典的无模型强化学习（MF RL）通常收敛缓慢且可能导致不安全的探索，而基于模型（MB）的方法计算开销大且对模型失配敏感。本文提出了一种受人类启发的混合强化学习架构，集成了巴甫洛夫、工具性无模型（Instrumental MF）和工具性基于模型（Instrumental MB）组件。受神经科学中巴甫洛夫和工具性学习的启发，该框架考虑了上下文无线电线索（此处指作为条件刺激 CS 的地理参考环境特征），以塑造内在价值信号并引导决策。学习过程通过专门的动机信号由内部动机驱动进一步调节。贝叶斯仲裁机制根据预测的可靠性自适应地融合 MF 和 MB 的估计值。仿真结果表明，与标准强化学习基准相比，该混合方法加速了学习，提高了运行安全性，并减少了在高不确定性区域的导航。巴甫洛夫条件反射促进了更安全的探索和更快的收敛，而仲裁机制实现了从探索到高效、计划驱动的利用（exploitation）的平滑过渡。总体而言，研究结果突显了生物启发模块化对于不确定性下鲁棒且自适应的自主系统的益处。
