...
## 摘要
使用生成式策略的同策略强化学习前景广阔，但仍未得到充分探索。一个核心挑战在于，传统的近端策略优化（PPO）是根据动作空间概率比率制定的，而基于扩散（diffusion）和流（flow）的策略则更自然地表示为轨迹级的生成过程。在这项工作中，我们提出了 GSB-PPO，这是一种受广义薛定谔桥（GSB）启发的生成式 PPO 的路径空间表述。我们的框架将 PPO 风格的近端更新从终端动作提升到完整的生成轨迹，为生成式策略的同策略优化提供了一个统一的视角。在该框架内，我们开发了两个具体的目标函数：基于裁剪的目标函数 GSB-PPO-Clip 和基于惩罚的目标函数 GSB-PPO-Penalty。实验结果表明，虽然这两个目标函数都与同策略训练兼容，但惩罚项表述在稳定性和性能上始终优于裁剪项表述。总之，我们的结果强调了路径空间近端正则化是使用 PPO 训练生成式策略的一种有效原则。
