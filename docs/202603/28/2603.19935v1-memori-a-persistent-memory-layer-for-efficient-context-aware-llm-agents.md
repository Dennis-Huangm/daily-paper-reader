...
tldr: "本研究针对LLM智能体在多轮对话中记忆持久化成本高、效率低的问题，提出了Memori。这是一个与模型无关的持久记忆层，通过将非结构化对话转化为语义三元组和摘要，实现了精准检索与推理。在LoCoMo基准测试中，Memori以极低的Token消耗（仅为全上下文的5%）实现了81.95%的准确率，显著降低了部署成本并提升了推理连贯性。"
selection_source: fresh_fetch
motivation: 现有的LLM记忆方案依赖注入大量原始对话，导致Token成本高昂、性能下降且存在供应商锁定问题。
method: 开发了一种高级增强流水线，将非结构化对话转换为紧凑的语义三元组和对话摘要，以结构化数据解决记忆存储问题。
result: "在LoCoMo基准测试中达到81.95%的准确率，Token消耗仅为全上下文的5%，比同类方案节省67%的成本。"
conclusion: 高效的智能体记忆应依赖于结构化数据表示而非单纯扩大上下文窗口，从而实现可扩展且低成本的部署。
---
