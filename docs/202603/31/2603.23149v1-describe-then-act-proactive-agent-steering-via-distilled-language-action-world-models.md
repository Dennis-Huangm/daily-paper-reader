...
tldr: 针对安全关键型智能体在执行动作前需预判后果但视觉模拟延迟高的问题，本文提出DILLO框架。该框架通过跨模态蒸馏，将视觉语言模型（VLM）的知识迁移到轻量级大语言模型（LLM）中，实现从“模拟后行动”到“描述后行动”的转变。DILLO仅需文本推理即可预测动作结果并引导策略，在保持高保真度的同时，推理速度提升14倍，显著提高了任务成功率。
selection_source: fresh_fetch
motivation: 传统的视觉世界模型在预测动作后果时推理延迟过高，难以满足实时安全控制的需求。
method: 提出DILLO框架，利用VLM作为教师模型标注轨迹，训练一个以潜变量为条件的LLM学生模型，通过纯文本路径预测语义结果。
result: 在MetaWorld和LIBERO基准测试中，DILLO实现了14倍的加速，并将任务平均成功率提升了9.3个百分点。
