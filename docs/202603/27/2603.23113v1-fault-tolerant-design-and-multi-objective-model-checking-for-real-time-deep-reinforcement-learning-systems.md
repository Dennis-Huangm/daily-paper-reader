...
tldr: 针对深度强化学习（DRL）在实时系统中因延迟等因素导致的可靠性挑战，本文提出一种容错设计框架。该框架利用时间自动机设计DRL与备用控制器间的切换逻辑，并将其转换为马尔可夫决策过程进行形式化分析。通过创新的多目标模型检测技术，在保证硬安全约束的同时优化性能，并开发了GPU加速工具MOPMC，显著提升了大规模模型的处理效率。
selection_source: fresh_fetch
motivation: 实时DRL系统面临延迟引发的故障及安全性挑战，且缺乏能同时优化性能与安全性的形式化分析方法。
method: 采用时间自动机设计切换逻辑并转换为MDP，结合新型凸查询技术进行多目标模型检测。
result: 开发了GPU加速工具MOPMC，在模型规模和目标数量的可扩展性上均优于现有方法。
conclusion: 该研究为实时DRL系统提供了严谨的容错设计与多目标优化手段，有效平衡了系统的安全性与性能。
---
