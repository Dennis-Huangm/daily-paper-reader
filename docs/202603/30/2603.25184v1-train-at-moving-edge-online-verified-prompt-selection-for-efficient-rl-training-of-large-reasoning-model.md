...
tldr: 针对大语言模型推理任务中强化学习（RL）训练开销巨大的问题，本文提出了一种名为HIVE的提示词选择框架。研究发现高价值样本集中在难度适中且不确定性高的“学习边缘”，且该边缘随训练动态变化。HIVE通过历史奖励轨迹进行粗选，并利用实时提示词熵进行精选，在显著提升Rollout效率的同时保持了模型性能。
selection_source: fresh_fetch
motivation: 强化学习训练中大量提示词产生的梯度贡献极低，导致了严重的计算资源浪费。
method: 提出HIVE框架，利用历史奖励轨迹进行初步筛选，并结合实时提示词熵作为代理指标来动态剔除低效样本。
result: 在多个数学推理基准测试中，HIVE在不损失模型性能的前提下，显著降低了训练过程中的计算开销。
conclusion: 动态选择处于“学习边缘”的高价值提示词是实现大语言模型高效强化学习训练的关键路径。
---
