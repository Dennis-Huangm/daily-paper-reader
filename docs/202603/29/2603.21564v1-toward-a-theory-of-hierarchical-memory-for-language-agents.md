...
method: 提出了由提取（映射原始数据）、粗化（分区并生成代表元）和遍历（根据预算选择内容）三个核心算子组成的理论框架。
result: 识别了代表元函数的自给自足光谱，并展示了该理论如何有效解释和分类包括文档层次结构、对话记忆在内的11种现有系统。
conclusion: 该理论为理解和优化语言智能体的分层记忆系统提供了通用的形式化基础，有助于未来更高效的记忆机制设计。
---
