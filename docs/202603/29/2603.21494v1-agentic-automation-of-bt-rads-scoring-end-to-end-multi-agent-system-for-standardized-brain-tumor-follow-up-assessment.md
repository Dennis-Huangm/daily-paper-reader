...
method: 采用多智能体LLM系统（包含临床变量提取和逻辑评分智能体）结合基于CNN的自动化肿瘤分割技术，对术后胶质瘤MRI进行端到端分类。
