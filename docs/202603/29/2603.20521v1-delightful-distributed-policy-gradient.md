...
tldr: 分布式强化学习常受过期或错误数据干扰，导致高惊奇度的失败案例误导策略更新。本文提出Delightful Policy Gradient (DG)，通过“delight”（优势值与惊奇度的乘积）对更新进行门控，抑制罕见失败并放大罕见成功。该方法无需行为概率，在存在数据污染和延迟的复杂任务中，其性能和计算效率显著优于传统策略梯度及重要性采样方法。
selection_source: fresh_fetch
motivation: 旨在解决分布式强化学习中高惊奇度数据（如过期或错误数据）导致的负面学习问题，特别是罕见失败对更新方向的误导。
method: 提出DG算法，利用优势值与惊奇度的乘积作为门控信号，在不依赖行为概率的情况下自适应地过滤无效失败并强化意外成功。
result: 在MNIST和Transformer序列任务中，DG在处理数据延迟、奖励损坏和代码漏洞等干扰时，表现出比重要性采样更强的鲁棒性和显著的计算优势。
conclusion: DG通过区分处理高惊奇度的成功与失败，为分布式强化学习提供了一种比传统重加权方法更高效、更稳健的梯度更新方案。
---

## 摘要
分布式强化学习在来自过时、有缺陷或不匹配的执行器（actor）的数据上进行训练，这些数据在学习器策略下会产生具有高惊奇度（surprisal，即负对数概率）的动作。核心困难不在于惊奇数据本身，而在于从惊奇数据中进行的“负向学习”。高惊奇度的失败可能会主导更新方向，尽管它们携带的有用信号很少；而高惊奇度的成功则揭示了当前策略原本会错过的机会。Delightful 策略梯度（DG）通过使用“愉悦度”（delight，即优势值与惊奇度的乘积）对每次更新进行门控来区分这些情况，在不使用行为概率的情况下抑制罕见的失败并放大罕见的成功。在受污染的采样下，标准策略梯度与真实梯度之间的余弦相似度会崩溃，而 DG 的余弦相似度则随着策略的改进而增长。任何不考虑符号的重加权方法（包括精确的重要性采样）都无法重现这种效果。在具有模拟过时性的 MNIST 任务上，不带离策校正的 DG 优于具有精确行为概率的重要性加权策略梯度。在具有过时性、执行器缺陷、奖励损坏和罕见发现的 Transformer 序列任务中，DG 实现了约 10 倍的误差降低。当所有四种摩擦力同时作用时，其计算优势达到了数量级级别，并随任务复杂度的增加而增长。
